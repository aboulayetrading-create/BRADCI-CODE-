      ...
      <section className="bg-white dark:bg-[#0B111D] border border-slate-200 dark:border-slate-800/90 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-sm dark:shadow-xl space-y-3.5">
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            <div className="relative flex items-center bg-slate-50 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 hover:border-amber-500/40 rounded-xl px-3 py-2.5 text-xs text-slate-900 dark:text-slate-200 w-full md:w-auto shadow-inner transition-colors">
              <MapPin className="w-4 h-4 text-amber-500 shrink-0 mr-2" />
              <select
                id="feed-commune-select"
                value={selectedCommune}
                onChange={(e) => setSelectedCommune(e.target.value)}
                className="bg-transparent text-slate-900 dark:text-white focus:outline-none text-xs font-semibold cursor-pointer w-full md:w-64 pr-2"
              >
                <option value="Toutes" className="bg-white dark:bg-slate-950 text-slate-900 dark:text-white font-bold">
                  {translate("📍 Toutes les Zones (Grand Abidjan & Environs)", "📍 All Zones (Greater Abidjan & Surrounds)")}
                </option>
                
                <optgroup label={translate("── Grand Abidjan (13 Communes) ──", "── Greater Abidjan (13 Communes) ──")} className="bg-slate-50 dark:bg-slate-950 text-amber-600 dark:text-amber-400 font-bold">
                  {COMMUNE_NAMES_ABIDJAN.map((c) => (
                    <option key={c} value={c} className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white font-normal">
                      {c}
                    </option>
                  ))}
                </optgroup>

                <optgroup label={translate("── Villes Métropolitaines & Balnéaires ──", "── Coastal & Metropolitan Cities ──")} className="bg-slate-50 dark:bg-slate-950 text-cyan-600 dark:text-cyan-400 font-bold">
                  {COMMUNE_NAMES_ENVIRONS.map((c) => (
                    <option key={c} value={c} className="bg-white dark:bg-slate-900 text-slate-900 dark:text-cyan-200 font-normal">
                      🌴 {c}
                    </option>
                  ))}
                </optgroup>
            ...
            {(searchQuery || selectedCategory !== 'Tous' || selectedCommune !== 'Toutes' || selectedFeedType !== 'all' || selectedVehicle !== 'Tous') && (
              <button
                ...
                className="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-amber-600 dark:text-amber-400 border border-slate-200 dark:border-slate-800 hover:border-amber-500/40 rounded-xl transition-all shrink-0 cursor-pointer shadow-sm"
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5 pt-2 border-t border-slate-200 dark:border-slate-800/60">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                selectedFeedType === 'all'
                  ...
                  : 'bg-slate-100 dark:bg-slate-900/90 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                selectedFeedType === 'shop'
                  ...
                  : 'bg-emerald-50 dark:bg-slate-900/90 text-emerald-800 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-slate-800 border border-emerald-200 dark:border-emerald-500/30'
              }`}
            >
              <Store className="w-3.5 h-3.5" />
              <span>{translate("🏪 Boutiques Officielles", "🏪 Official Stores")}</span>
              <span className={`text-[10px] px-1.5 py-0.2 rounded-md font-mono-num font-extrabold ${
                selectedFeedType === 'shop' ? 'bg-emerald-800 text-white' : 'bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300'
              }`}>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                selectedFeedType === 'b2b'
                  ...
                  : 'bg-blue-50 dark:bg-slate-900/90 text-blue-800 dark:text-cyan-300 hover:bg-blue-100 dark:hover:bg-slate-800 border border-blue-200 dark:border-blue-500/30'
              }`}
            >
              <Building2 className="w-3.5 h-3.5 text-cyan-500" />
              <span>{translate("🏢 Déstockage B2B", "🏢 B2B Lots")}</span>
              <span className={`text-[10px] px-1.5 py-0.2 rounded-md font-mono-num font-extrabold ${
                selectedFeedType === 'b2b' ? 'bg-blue-800 text-white' : 'bg-blue-100 dark:bg-slate-800 text-blue-800 dark:text-cyan-300'
              }`}>
          ...
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 p-1 rounded-xl text-xs text-slate-600 dark:text-slate-400">
            <span className="text-[11px] font-semibold px-2 text-slate-600 dark:text-slate-400 hidden sm:inline">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer ${
                selectedVehicle === 'Tous' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm border border-slate-200 dark:border-slate-700' : 'hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              {translate("Tous", "All")}
            </button>
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer ${
                selectedVehicle === 'moto' ? 'bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 border border-emerald-500/40 shadow-sm' : 'hover:text-slate-900 dark:hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer ${
                selectedVehicle === 'cargo' ? 'bg-purple-500/20 text-purple-800 dark:text-purple-300 border border-purple-500/40 shadow-sm' : 'hover:text-slate-900 dark:hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold shrink-0 transition-all cursor-pointer ${
                selectedCategory === cat.key
                  ...
                  : 'bg-slate-100 dark:bg-slate-900/80 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {selectedCommune !== 'Toutes' && (
              <span className="text-xs bg-amber-500/15 text-amber-800 dark:text-amber-300 border border-amber-500/30 px-2.5 py-0.5 rounded-full font-semibold flex items-center gap-1">
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  ...
                  className={`group bg-white dark:bg-[#0C121E] border ${
                    isOutOfStock
                      ? 'border-red-300 dark:border-red-900/40 opacity-85 hover:border-red-500/50'
                      : isShop 
                        ? 'border-slate-200 dark:border-slate-800/90 hover:border-emerald-500/50 hover:shadow-emerald-500/10' 
                        : 'border-slate-200 dark:border-slate-800/90 hover:border-amber-500/50 hover:shadow-amber-500/10'
                  } rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-200 cursor-pointer flex flex-col justify-between`}
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      {/* Product Title */}
                      <h4 className="font-bold text-sm text-slate-900 dark:text-white line-clamp-2 leading-snug group-hover:text-amber-500 dark:group-hover:text-amber-300 transition-colors">
                        {product.title}
                      </h4>
                      ...
                      <div className="flex items-center gap-1.5 mt-2.5 text-[11px] text-slate-500 dark:text-slate-400">
                        <button
                          ...
                          className={`truncate hover:underline flex items-center gap-1 transition-colors ${
                            isShop ? 'text-emerald-700 dark:text-emerald-400 hover:text-emerald-600 font-bold' : 'text-slate-700 dark:text-slate-300 hover:text-amber-500 dark:hover:text-amber-400'
                          }`}
                          title={translate("Voir la vitrine boutique", "View official storefront")}
                        >
                          <Store className={`w-3 h-3 shrink-0 ${isShop ? 'text-emerald-500' : 'text-amber-500'}`} />
                          <span className="truncate">{product.shopName || product.sellerName}</span>
                        </button>
                        {isShop ? (
                          <span className="text-[9px] bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30 px-1.5 py-0.2 rounded font-bold shrink-0">
                            {translate("Boutique", "Store")}
                          </span>
                        ) : product.sellerPlan === 'pro' ? (
                          <span className="text-[9px] bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 px-1.5 py-0.2 rounded font-bold shrink-0 flex items-center gap-0.5">
                            <Crown className="w-2.5 h-2.5 text-amber-500" />
                            <span>VIP</span>
                          </span>
                        ) : product.sellerPlan === 'standard' ? (
                          <span className="text-[9px] bg-blue-500/20 text-blue-700 dark:text-blue-300 border border-blue-500/30 px-1.5 py-0.2 rounded font-bold shrink-0 flex items-center gap-0.5">
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/90">
                      {/* Price header row */}
                      <div className="flex items-end justify-between mb-3">
                          ...
                          <div>
                            <div className="flex items-baseline gap-1">
                              <span className={`text-lg sm:text-xl font-black font-mono-num ${
                                isShop ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'
                              }`}>
                        ...
                        <div className="text-right">
                          {isShop ? (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30">
                      ...
                      {isDriver ? (
                        ...
                        <button
                          ...
                          className="w-full py-2.5 rounded-xl bg-emerald-500/20 hover:bg-emerald-500 text-emerald-800 dark:text-emerald-300 hover:text-white dark:hover:text-slate-950 border border-emerald-500/40 text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                        >
                        ...
                        <div className="space-y-1.5">
                          <button
                            type="button"
                            disabled
                            className="w-full py-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-red-500/30 text-red-500 dark:text-red-400/80 text-xs font-bold flex items-center justify-center gap-1.5 cursor-not-allowed"
                          >
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className={`px-3 py-2.5 rounded-xl border text-xs font-bold transition-all flex items-center justify-center gap-1 cursor-pointer shrink-0 ${
                                recentlyAddedId === product.id
                                  ? 'bg-emerald-600/30 text-emerald-800 dark:text-emerald-300 border-emerald-500/60'
                                  : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border-slate-200 dark:border-slate-700'
                              }`}
                            ...
                            <button
                              ...
                              className="px-2.5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white border border-slate-200 dark:border-slate-800 transition-all cursor-pointer shrink-0"
                          ...
                          {/* Reassurance text */}
                          <div className="flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 px-0.5 font-medium">
