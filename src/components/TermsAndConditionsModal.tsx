  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/85 backdrop-blur-md animate-in fade-in overflow-y-auto">
      <div 
        ...
        className="w-full max-w-3xl bg-[#0C121E] border border-slate-800 rounded-3xl p-5 sm:p-8 shadow-2xl relative my-auto max-h-[90vh] flex flex-col text-slate-200"
      >
        {/* Close Button */}
        <button
        ...
        <div className="flex items-center gap-3 mb-4 pb-4 border-b border-slate-800">
          ...
          <div>
            <div className="inline-flex items-center gap-1.5 bg-amber-500/10 text-amber-400 px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-amber-500/20 mb-1">
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-4 text-xs text-slate-300 bg-slate-950/60 p-4 rounded-2xl border border-slate-800/80 leading-relaxed font-sans select-text">
          {BRAD_CI_TERMS.rules.map((rule, idx) => (
            <section key={rule.id} className={`space-y-1.5 ${idx > 0 ? 'pt-3 border-t border-slate-800/60' : ''}`}>
              <h3 className="font-bold text-amber-400 text-sm flex items-center gap-1.5">
                {idx === 0 ? <ShieldCheck className="w-4 h-4 text-emerald-400" /> :
                 idx === 1 ? <PhoneOff className="w-4 h-4 text-rose-400" /> :
                 idx === 2 ? <AlertOctagon className="w-4 h-4 text-amber-400" /> :
                 idx === 3 ? <Store className="w-4 h-4 text-blue-400" /> :
        ...
        {/* Action Footer */}
        <div className="mt-4 pt-4 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              onClick={() => setTermsModalOpen(false)}
              className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-bold border border-slate-700 transition-colors"
            >
